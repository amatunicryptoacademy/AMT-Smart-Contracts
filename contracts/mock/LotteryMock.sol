...
contract LotteryMock is Ownable {
    ...
    constructor(IERC20 token_, address priceFeed_, IApePair pair_) {
        token = token_;
    ...
    function getNumberOfMatching(
        uint16[] memory usersNumbers_,
        uint16[] memory winningNumbers_
    ) public pure returns (uint16[] memory wonNumbers) {
    ...
    function _split(uint256 lotteryId_) public returns (uint16[] memory) {
        ...
        while (numbersCount < 3) {
            ...
            if (uint16(numberRepresentation.mod(5)) > 0) {
                ...
                if (duplicates == 0) {
                    winningNumbers[numbersCount] = uint16(numberRepresentation.mod(5));
                    numbersCount++;
                }
            }
            i++;
